(ns metalfabmfg.operation
  "MetalFabOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (MetalFabAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Metal
  Fabrication Plant Operations Governor (:govern) and the rollout
  ...
  (:require [langgraph.graph :as g]
            ...
            [metalfabmfg.advisor :as advisor]
...
(defn build
  "Compiles a MetalFabOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `metalfabmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MetalFabAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Metal Fabrication Plant Operations Governor -- independent censor.
      (g/add-node :govern
