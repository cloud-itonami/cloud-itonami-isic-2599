(ns metalfabmfg.advisor
  "MetalFabAdvisor -- the *contained intelligence node* for the
  metal-fabrication shop plant-operations coordination actor (ISIC
  2599, other fabricated metal products n.e.c.).

  It normalizes production-batch patches (product-category/weight/
  defect-rate), drafts a stamping-press/pressing-line/wire-forming-
  machine maintenance scheduling proposal against a piece of
  equipment, drafts a safety-concern flag, and drafts an outbound
  fabricated-metal-product shipment coordination proposal against a
  ...
  a committed record and NEVER a real stamping/pressing-line actuation
  or freight dispatch. Every output is censored downstream by
  `metalfabmfg.governor` before anything touches the SSoT -- see
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `metalfabmfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct stamping/
  ...
  `metalfabmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  "Production-batch (stamped/pressed/wire-formed lot) intake upsert --
  the advisor only normalizes/validates the patch; it does not invent
  the batch's product-category, weight, defect-rate or verification
...
(defn- schedule-maintenance
  "Draft a stamping-press/pressing-line/wire-forming-machine
  ...
  registered?) in its rationale, but `metalfabmfg.governor` NEVER
    ...
    {:summary    (str subject " 向け保守作業予定提案 (" (:maintenance-type value) ")"
     ...
     :rationale  (if eq
                   (str "equipment-verified?=" (registry/equipment-verified? eq)
                        ...
                        " actuate-press-line?=" (boolean (:actuate-press-line? value)))
                   (str equipment-id " が見つかりません"))
     :cites      (if eq [equipment-id] [])
     :effect     :maintenance/schedule
     :value      value
     :stake      nil
     :confidence (if (and ready? (not (:actuate-press-line? value))) 0.9 0.3)}))

(defn- flag-safety-concern
  "Draft a sharp-edge/burr-laceration (freshly stamped/pressed sheet
  metal)/press-pinch-point/wire-entanglement/coating-lubricant-
  chemical-exposure/equipment-safety concern. ALWAYS `:stake
  ...
  `metalfabmfg.phase`: no phase ever adds this op to a phase's `:auto`
  set; `metalfabmfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound fabricated-metal-product shipment coordination
  proposal against a production batch. The advisor passes through the
  caller's own claimed weight -- it does NOT invent one, and
  `metalfabmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは金属加工(スタンピング・プレス成形・ワイヤー成形)工場"
       ...
       "スタンピングプレス・成形ライン等の設備の直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `metalfabmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :metalfabmfg-advisor-proposal
   :op         (:op request)
