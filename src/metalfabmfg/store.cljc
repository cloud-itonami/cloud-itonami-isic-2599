(ns metalfabmfg.store
  "SSoT for the metal-fabrication shop plant-operations coordination
  ...
  self-contained (no external fabricated-metal-products capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch (a
                             stamped/pressed/wire-formed lot)'s
                             ...
                             ground truth.
    - `equipment`         -- a stamping-press/pressing-line/wire-
                             forming-machine unit's own record.
    ...
    - `maintenance`       -- a scheduled stamping/pressing-line/wire-
                             forming-equipment maintenance-window DRAFT
                             against a piece of equipment
                             (`metalfabmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound fabricated-metal-
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (metalfabmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (metalfabmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-category :stamped-metal-part :process-type :stamping
                :material "Stamped Steel Bracket Lot"
                :weight-kg 20000.0 :defect-rate-percent 1.1
                :verified? true :registered? true
                :shipped-weight-kg 4000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-category :wire-product :process-type :wire-forming
                :material "Galvanized Wire Mesh Coil Lot"
                :weight-kg 6000.0 :defect-rate-percent 0.4
                :verified? true :registered? true
                :shipped-weight-kg 5600.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-category :metal-household-good :process-type :pressing
                :material "Pressed Steel Cookware Lot"
                :weight-kg 9000.0 :defect-rate-percent 1.8
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"press-001" {:id "press-001" :kind :stamping-press
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "wireform-002" {:id "wireform-002" :kind :wire-forming-machine
                   :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `metalfabmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `metalfabmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered stamping-press unit
  (schedulable for maintenance), one UNVERIFIED/unregistered
  wire-forming-machine unit (blocks any maintenance scheduling against
