(ns metalfabmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the stamping press or pressing/
  wire-forming line directly... does NOT authorize or execute
  maintenance/press-line operations') implemented faithfully. The
  single invariant under test:

    MetalFabAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Metal Fabrication Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-category :stamped-metal-part}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :stamped-metal-part (:product-category (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-press-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest press-line-actuate-is-held-and-permanently-blocked
  (testing "a proposal that sets :actuate-press-line? true -> HOLD, PERMANENT, never reaches request-approval even though the equipment is verified and registered"
    (let [[db actor] (fresh)
      ...
      (is (not= :interrupted (:status res)))
      (is (some #{:press-line-actuate-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))
  ...
  (testing "scheduling the SAME maintenance record twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          _ (exec-op actor "t9a" {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                                  :value {:equipment-id "press-001" :maintenance-type :die-inspection
                                          :scheduled-date "2026-08-01" :actuate-press-line? false}} coordinator)
          _ (approve! actor "t9a")
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "press-001" :severity :moderate
                                            :description "stamping press pinch-point guard loose"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
